...
class MemberInvestmentsTable
{
	...
	public function findInvestors($branch, $investmentId=null){
		$resultSet = $this->tableGateway->select(function(Select $select) use ($branch, $investmentId){
			...
			$select->join('members', 'member_investments.member_id = members.member_id', array('firstname', 'lastname', 'member_id', 'emailid', 'gardian_name','address','nominee_name','nominee_relation'));
