...
class NumberToWords
{
	public static function toWords($number) {
		...
		if (($number >= 0 && (int) $number < 0) || (int) $number < 0 - PHP_INT_MAX) {
			// overflow
			trigger_error(
				'self::toWords only accepts numbers between -' . PHP_INT_MAX . ' and ' . PHP_INT_MAX,
				E_USER_WARNING
			);
			return false;
		}
		...
		$string = str_replace(array('point zero zero','point'), array('and paise zero','and paise'), $string);
		return $string;
	}
}
